...
export function PerformanceReviewModal({ isOpen, onClose, review }: PerformanceReviewModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto flex flex-col">
        <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center sticky top-0 bg-white dark:bg-gray-900 z-10">
          <h2 className="text-xl font-bold text-navy-900 dark:text-white">
            Performance Review - {review.reviewPeriod}
          </h2>
        ...
        <div className="p-6 space-y-6 flex-1">
          ...
          <form className="space-y-6">
            <div className={`space-y-4 ${status !== 'EMPLOYEE_REVIEW' ? 'opacity-70 pointer-events-none' : ''}`}>
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Employee Comments</label>
                <textarea name="employeeComments" value={formData.employeeComments || ''} onChange={handleChange} className="w-full rounded-md border border-gray-300 dark:border-gray-700 p-2 dark:bg-gray-900" rows={3} />
              </div>
              <div className="grid grid-cols-2 gap-4">
            ...
            {status !== 'EMPLOYEE_REVIEW' && (
              <div className={`space-y-4 ${status !== 'MANAGER_REVIEW' ? 'opacity-70 pointer-events-none' : ''}`}>
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">Manager Comments</label>
                  <textarea name="managerComments" value={formData.managerComments || ''} onChange={handleChange} className="w-full rounded-md border border-gray-300 dark:border-gray-700 p-2 dark:bg-gray-900" rows={3} />
                </div>
            ...
            {(status === 'HR_REVIEW' || status === 'FINAL_APPROVAL' || status === 'COMPLETED') && (
              <div className={`space-y-4 ${status !== 'HR_REVIEW' ? 'opacity-70 pointer-events-none' : ''}`}>
                ...
                <div>
                  ...
                  <textarea name="hrComments" value={formData.hrComments || ''} onChange={handleChange} className="w-full rounded-md border border-gray-300 dark:border-gray-700 p-2 dark:bg-gray-900" rows={3} />
                </div>
                {canSubmitHR && (
              ...
              <div className={`space-y-4 ${status !== 'FINAL_APPROVAL' ? 'opacity-70 pointer-events-none' : ''}`}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select name="finalApprovalStatus" value={formData.finalApprovalStatus || 'APPROVAL_PENDING'} onChange={handleChange} className="flex h-10 w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-gray-900 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-accent-500">
